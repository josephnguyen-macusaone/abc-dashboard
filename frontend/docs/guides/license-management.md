...
### 2.5 Column widths (single source)

Column widths for both the **DataTable** and **DataGrid** are defined in one place so resizing applies to both views:

- **File:** `shared/constants/license.ts` — exports `LICENSE_COLUMN_WIDTHS` (column id → `{ size, minSize? }`).
- **Usage:** `license-table-columns.tsx` and `license-grid-columns.tsx` spread the width for each column (e.g. `...LICENSE_COLUMN_WIDTHS.dba`).

To resize a license column, edit `license.ts`.
...
| Column widths (table + grid) | `shared/constants/license.ts` (`LICENSE_COLUMN_WIDTHS`) |
