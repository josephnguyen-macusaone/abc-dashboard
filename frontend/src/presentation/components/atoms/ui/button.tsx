...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      ...
      size: {
        // MAC USA ONE Typography: Button sizes with appropriate text styles
        xs: "h-8 px-3 py-2 has-[>svg]:px-2.5 text-button-s",
        default: "h-9 px-4 py-2 has-[>svg]:px-3 text-button-m",
        md: "h-10 px-5 py-2.5 has-[>svg]:px-4 text-button-m",
        sm: "h-8 rounded-md gap-1.5 px-3 has-[>svg]:px-2.5 text-button-s",
        lg: "h-11 rounded-md px-8 has-[>svg]:px-6 text-button-l",
        xl: "h-12 rounded-md px-10 has-[>svg]:px-8 text-button-xl",
        icon: "size-9",
        "icon-sm": "size-8",
        "icon-md": "size-10",
        "icon-lg": "size-11",
