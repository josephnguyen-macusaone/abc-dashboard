...
import { useDebouncedCallback } from "@/presentation/hooks";
import {
  getDefaultOperator,
    ...
    >
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent
          ...
          className={cn(
            "flex w-full max-w-[var(--radix-popover-content-available-width)] flex-col gap-3.5 p-4 sm:min-w-[480px]",
            className,
          )}
          {...props}
        >
  ...
  return (
    <SortableItem value={filter.id} asChild>
      ...
      >
        ...
        >
          <SelectTrigger className="h-8 w-32 rounded lowercase">
...
function DataGridFilterInput<TData>({
  ...
  if (isSelectVariant && selectOptions && selectOptions.length > 0) {
    const inputListboxId = `${inputId}-listbox`;
