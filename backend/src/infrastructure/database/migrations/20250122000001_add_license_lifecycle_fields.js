...
export async function up(knex) {
  logger.info('Adding license lifecycle management fields...');
  ...
  await knex.schema.alterTable('licenses', (table) => {
    ...
    table
      .jsonb('renewal_reminders_sent')
      .defaultTo('[]')
      .comment('Array of sent reminder types (30days, 7days, 1day)');
    table
      .timestamp('last_renewal_reminder', { useTz: true })
      .nullable()
      .comment('Timestamp of last renewal reminder sent');
    table
      .timestamp('renewal_due_date', { useTz: true })
      .nullable()
      .comment('Calculated date when renewal is due');

    // Auto-suspension and grace period settings
    table
      .boolean('auto_suspend_enabled')
      .defaultTo(false)
      .comment('Whether to automatically suspend expired licenses');
    table
      .integer('grace_period_days')
      .defaultTo(30)
      .comment('Number of days grace period after expiration');
    table
      .timestamp('grace_period_end', { useTz: true })
      .nullable()
      .comment('Calculated end of grace period');

    // Suspension and reactivation tracking
    table.text('suspension_reason').nullable().comment('Reason for license suspension');
    table
      .timestamp('suspended_at', { useTz: true })
      .nullable()
      .comment('When license was suspended');
    table
      .timestamp('reactivated_at', { useTz: true })
      .nullable()
      .comment('When license was reactivated');
  ...
  logger.info('License lifecycle fields added successfully');
}
...
export async function down(knex) {
  logger.info('Removing license lifecycle management fields...');
  ...
  logger.info('License lifecycle fields removed successfully');
}
