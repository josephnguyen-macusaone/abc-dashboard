...
- The `agents_name` column expects valid JSON (e.g. a JSON array like `["Amy Miller"]`).
- The value `{"Amy Miller"}` is invalid (object with a key and no value).
- In the **current** seed, `agents_name` is set with `JSON.stringify(agentsName)` where `agentsName` is an array, which produces valid JSON. So again, this points to an **older seed** (or older insert path) that sometimes sent invalid JSON for `agents_name`.
...
- **Why did migrations/seeds fail?**
  The logs are consistent with **old Docker images**: an older migration that re-created `idx_external_licenses_coming_expired`, and older seed/insert logic that could write invalid JSON for `agents_name`.

**What you should do:**

1. **Rebuild backend (and frontend if needed) images** so they contain the current code (migration that doesn’t create the duplicate index, seed that uses `JSON.stringify(agentsName)` correctly).
